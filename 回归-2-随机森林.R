# 模型机器---R语言tidymodels包机器学习分类与回归模型---回归---随机森林
...
hpgrid_rf <- grid_regular(hpset_rf, levels = c(3, 2))
hpgrid_rf
...
varImpPlot(final_rf2, main = "varimpplot", type = 1)
# 偏依赖图
...
predtest_rf %>%
  ...
  geom_abline(intercept = 0, slope = 1, color = "red", linewidth = 1.2) +
  labs(x = "实际值", y = "预测值", 
       title = "随机森林模型在测试集上的预测效果") +
  theme_bw()

#############################################################

# 合并结果
eval_rf <- predtrain_rf %>%
...
eval_best_cv5_rf <- tune_rf %>%
  ...
  metrics(truth = label, estimate = .pred) %>%
  mutate(model = "rf") %>%
  inner_join(eval_best_cv_rf[c(3,5,7)])
eval_best_cv5_rf
...
save(final_rf,
     ...
     eval_rf,
     eval_best_cv5_rf, 
     file = ".\\model\\evalresult_rf.RData")
...
force_plot(object = shap[1L, ], 
           ...
           baseline = mean(predtrain_rf$.pred),
           display = "viewer") 
