# 模型机器---R语言tidymodels包机器学习分类与回归模型---多分类---随机森林
...
hpgrid_rf <- grid_regular(hpset_rf, levels = c(2, 3, 2))
hpgrid_rf
...
varImpPlot(final_rf2, main = "变量重要性", type = 2)
# 偏依赖图
...
force_plot(object = shap[1L, ], 
           ...
           baseline = mean(predtrain_rf$.pred_H),
           display = "viewer") 
