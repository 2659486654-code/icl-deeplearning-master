# 模型机器---R语言tidymodels包机器学习分类与回归模型---多分类---决策树
...
# hpgrid_dt <- grid_regular(hpset_dt, levels = c(3, 2, 3))
set.seed(42)
hpgrid_dt <- grid_random(hpset_dt, size = 10)
hpgrid_dt
...
levels(predtrain_dt$.pred_class) <- levels(predtrain_dt$label)
predtrain_dt
...
roctrain_dt <- predtrain_dt %>%
  roc_curve(label, one_of(roc_cols)) %>%
  mutate(dataset = "train")
...
predtest_dt <- final_dt %>%
  ...
  mutate(model = "dt")
# 将预测分类的levels设定成因变量实际值的levels
levels(predtest_dt$.pred_class) <- levels(predtest_dt$label)
predtest_dt
# 评估模型ROC曲线-测试集上

roc_cols <- paste(".pred_", levels(predtest_dt$label), sep = "")
...
force_plot(object = shap[1L, ], 
           ...
           baseline = mean(predtrain_dt$.pred_H),
           display = "viewer") 
